...
function BulletSection({ icon, title, items, accent }: { icon: string; title: string; items?: string[]; accent?: string }) {
  if (!items || items.length === 0) return null;
  return (
    <div className="jdetail-section jdetail-section--rich">
      <h2 className="jdetail-section-title jdetail-section-title--rich">
        <span className="jdetail-section-icon">{icon}</span>{title}
      </h2>
      <ul className="jdetail-bullet-list">
        {items.map((item, i) => (
          <li key={i} className="jdetail-bullet-item">
            <span className="jdetail-bullet-dot" style={accent ? { background: accent } : undefined} aria-hidden="true" />
            {item}
          </li>
        ))}
      </ul>
...
// ── Tag-pill section (skills, products, certs) ──────────────────────
function TagSection({ icon, title, items, tagClass }: { icon: string; title: string; items?: string[]; tagClass: string }) {
  if (!items || items.length === 0) return null;
  return (
    <div className="jdetail-section jdetail-section--rich">
      <h2 className="jdetail-section-title jdetail-section-title--rich">
      ...
      <div className="job-card__skills">
        {items.map(item => <span key={item} className={tagClass}>{item}</span>)}
      </div>
    </div>
  );
}
...
export default function JobDetailPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res  = await fetch(`${API}/api/saved-jobs/ids`, { headers: { Authorization: `Bearer ${token}` } });
        const data = await res.json();
      ...
      <div className="jdetail-page">
        <div className="jdetail-container">
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 320px", gap:"1.5rem", marginTop:"1.5rem" }}>
            <div><div className="jdetail-skeleton" style={{ height:32, marginBottom:12, borderRadius:8 }} /><div className="jdetail-skeleton" style={{ height:400, borderRadius:10 }} /></div>
  ...
  return (
    <>
          ...
          <div className="jdetail-container">
            ...
            <div className="jdetail-hero__inner">
              <div className="jdetail-hero__left">
                ...
                <div>
                  ...
                  <p className="jdetail-company">
                    ...
                    {job.company?.website && (
                      <a href={job.company.website} target="_blank" rel="noopener noreferrer" className="jdetail-company__link">↗ Visit website</a>
                    )}
                  </p>
              ...
              <div className="jdetail-hero__actions">
                <button className="btn btn--primary btn--lg" onClick={() => { window.open(job.applyUrl, "_blank", "noopener,noreferrer"); setShowApplyModal(true); }}>
                  Apply Now →
                </button>
                {user && (
                ...
                <button
                  ...
                  onMouseEnter={e => { (e.currentTarget as HTMLButtonElement).style.background = "#ffffff"; (e.currentTarget as HTMLButtonElement).style.color = "#0070C0"; }}
        ...
        <div className="jdetail-container jdetail-body">
          <div className="jdetail-grid">
            <main className="jdetail-main">
              ...
              <div className="jdetail-info-row">
                {[
                  ...
                  <div key={item.label} className="jdetail-info-card">
                    ...
                    <div><span className="jdetail-info-label">{item.label}</span><span className="jdetail-info-value">{item.value}</span></div>
                  </div>
                ))}
              </div>
              ...
              {/* Overview */}
              {job.overview && (
                <div className="jdetail-overview-card">
                  <p>{job.overview}</p>
                </div>
              )}

              {/* Structured sections — only render if data exists */}
              <BulletSection icon="🎯" title="Responsibilities"          items={job.responsibilities}        accent="#00A1E0" />
              <BulletSection icon="✅" title="Requirements"               items={job.requirements}            accent="#38A169" />
              <BulletSection icon="⭐" title="Preferred Qualifications"  items={job.preferredQualifications} accent="#F6A623" />
              <BulletSection icon="🎁" title="Benefits"                  items={job.benefits}                accent="#8B5CF6" />

              <TagSection icon="🛠️" title="Skills"              items={job.skills}             tagClass="skill-tag jdetail-skill" />
              <TagSection icon="☁️" title="Salesforce Products" items={job.salesforceProducts} tagClass="jdetail-product-tag" />
              <TagSection icon="🏅" title="Certifications"      items={job.certifications}     tagClass="jdetail-cert-tag" />

              {/* Fallback — raw description only if no structured data at all */}
              {!job.overview && !job.responsibilities?.length && !job.requirements?.length && (
              ...
              <div className="jdetail-apply-cta">
                ...
                <a href={job.applyUrl} target="_blank" rel="noopener noreferrer" className="btn btn--primary btn--lg">Apply Now →</a>
              </div>
            </main>
            ...
            <aside className="jdetail-sidebar">
              <div className="jdetail-sidebar-card">
                ...
                <a href={job.applyUrl} target="_blank" rel="noopener noreferrer" className="btn btn--primary" style={{width:"100%",justifyContent:"center",marginTop:"1.25rem"}}>Apply Now →</a>
                <button className={`btn ${saved ? "btn--saved" : "btn--ghost"}`} style={{width:"100%",justifyContent:"center",marginTop:"0.625rem"}} onClick={handleSave} disabled={saving}>
                ...
                {user && (
                  <button className="btn btn--ghost ats-check-btn" style={{ width: "100%", justifyContent: "center", marginTop: "0.625rem" }} onClick={() => setShowAts(true)}>
                    🎯 Check ATS Score
                  </button>
                )}
              </div>
              ...
              <div className="jdetail-sidebar-card">
                ...
                <div className="jdetail-share-btns">
                  ...
                  ].map(btn => <button key={btn.label} className="btn btn--ghost btn--sm jdetail-share-btn" onClick={btn.action}>{btn.label}</button>)}
                </div>
              </div>

              <button className="btn btn--ghost" style={{width:"100%",justifyContent:"center"}} onClick={() => router.push("/jobs")}>← Back to Jobs</button>
            </aside>
          </div>
        </div>
      </div>
      ...
      {showApplyModal && (
        <ApplyConfirmModal jobId={job._id} jobTitle={job.title} applyUrl={job.applyUrl} onClose={() => setShowApplyModal(false)} onMarked={() => {}} />
      )}
      <Footer />
    </>
  );
